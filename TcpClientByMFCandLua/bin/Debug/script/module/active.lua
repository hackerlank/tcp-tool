...
function init_reg_recv_callback()
	...
	for msgno,callback_desc in pairs(event) do
		modulemgr:reg_recv_callback(msgno, print_table, callback_desc)
	end
end
...
function Test_get_activity_reward()
	...
	modulemgr:send(data.m, data.p, tableutil.print_table, "on_get_activity_reward")
end
-------------------------------
...
function Test_get_activity_remain_time()
	...
	modulemgr:send(data.m, data.p, tableutil.print_table, "on_get_activity_remain_time")
end
-------------------------------
-- 请求获取活动剩余时间 ACTIVE_GET_REMAIN_TIME 10003
-- sample:{m=10007, p = {}}
function Test_get_buy_level_fund()
	...
	modulemgr:send(data.m, data.p, tableutil.print_table, "on_get_buy_level_fund")
end
-------------------------------
